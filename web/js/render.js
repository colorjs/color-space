...
const ent = (s, lite, st) => { const cls = classify(s)
	...
	const full = vals && !lite   // lead cards bake sliders + thumb positions; sheet rows bake values only
	return `<article class="ent${lite ? ' lite' : ''}" data-s="${s}"${vals ? ` data-v="${st.hx}${full ? '' : ':l'}"${full ? ` data-g="${st.hx}:0:srgb"` : ''}` : ''} style="--nch:${cls.ch.length}">
	 ...
	 <div class="chs">${cls.ch.map((c2, i) => `<div class="ch" data-i="${i}" title="${cname(c2)}"${full ? ` style="background:linear-gradient(90deg, ${ramp(s, vals, i, c2.min, c2.max, 48, 'srgb').join(',')}), var(--checker)"` : ''}><input type="range" class="nrg" data-i="${i}" min="${c2.min}" max="${c2.max}" step="any"${full ? ` value="${vals[i]}" style="--tkc:${st.hx};--tki:${st.ink}"` : ''} tabindex="-1" aria-label="${cname(c2)} slider"></div>`).join('')}</div>
	</article>` }
...
export const catHTML = (m, secs = sections) => { let st = null
	...
	return `<nav class="toc" id="toc">${secs.map((c, i) => `<i class="tsp"></i><div class="ti" data-i="${i}"><button class="tn"${c.tip ? ` data-tip="${c.tip}"` : ''}>${c.name}<span class="tc tnum">${c.spaces.length}</span></button></div>`).join('')}<div class="gtabs" aria-label="group the catalog by">${['family', 'purpose', 'era'].map(g => `<button class="gtag${g === 'family' ? ' on' : ''}" data-g="${g}" aria-pressed="${g === 'family'}">${g[0].toUpperCase() + g.slice(1)}</button>`).join('')}</div></nav><div class="secs" id="secs">` + secs.map((c, si) => `<section class="sec" data-sec><h2 class="shw"${c.tip ? ` data-tip="${c.tip}" tabindex="0"` : ''}>${c.name}<span class="c tnum">${c.spaces.length}</span>${si === 0 ? `<select class="gsel" aria-label="group the catalog by">${['family', 'purpose', 'era'].map(g => `<option value="${g}"${g === 'family' ? ' selected' : ''}>${g[0].toUpperCase() + g.slice(1)}</option>`).join('')}</select>` : ''}</h2><div class="grid">
