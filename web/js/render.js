...
export const catHTML = (m, secs = sections) => { let st = null
	...
	return `<nav class="toc" id="toc">${secs.map((c, i) => `<i class="tsp"></i><div class="ti" data-i="${i}"><button class="tn"${c.tip ? ` data-tip="${c.tip}"` : ''}>${c.name}<span class="tc tnum">${c.spaces.length}</span></button></div>`).join('')}<div class="gtabs" aria-label="group the catalog by">${['family', 'purpose', 'era'].map(g => `<button class="gtag${g === 'family' ? ' on' : ''}" data-g="${g}" aria-pressed="${g === 'family'}">${g}</button>`).join('')}</div></nav><div class="secs" id="secs">` + secs.map((c, si) => `<section class="sec" data-sec><h2 class="shw"${c.tip ? ` data-tip="${c.tip}" tabindex="0"` : ''}>${c.name}<span class="c tnum">${c.spaces.length}</span>${si === 0 ? `<select class="gsel" aria-label="group the catalog by">${['family', 'purpose', 'era'].map(g => `<option${g === 'family' ? ' selected' : ''}>${g}</option>`).join('')}</select>` : ''}</h2><div class="grid">
