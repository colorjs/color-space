...
try {
	...
	await motion.evaluate(()=>{ const orig=createImageBitmap; window.__bitmapFlight={active:0,max:0,orig}; window.createImageBitmap=(...args)=>{ const s=window.__bitmapFlight; s.active++; s.max=Math.max(s.max,s.active)
	...
	await motion.mouse.move(mr.x+mr.width*.25,mr.y+mr.height*.55); await motion.mouse.down(); await motion.mouse.move(mr.x+mr.width*.7,mr.y+mr.height*.3,{steps:24}); await motion.mouse.up()
	...
	assert.equal(maxBitmaps<=1,true,'dossier dragging keeps only one shared-kernel bitmap transfer in flight')
