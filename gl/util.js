 ...
 */
export default [
	{ name: 'cbrt_', code: `float cbrt_(float x) { return sign(x) * pow(abs(x), 1.0 / 3.0); }` },
	{ name: 'spow_', code: `float spow_(float x, float p) { return sign(x) * pow(abs(x), p); }` },
	{ name: 'atan2_', code: `float atan2_(float y, float x) { return atan(y, x); }` },
	{ name: 'mod_', code: `float mod_(float x, float y) { return x - y * floor(x / y); }` },
	// cartesian opponent pair -> cylindrical [L, C, h°]; mirrors util.js cartToPolar
	{ name: 'polar_fwd', code: `vec3 polar_fwd(vec3 c) {
	...
	{ name: 'polar_inv', code: `vec3 polar_inv(vec3 c) {
