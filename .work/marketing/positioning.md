...
Three words, three proofs: *Every* (162, more than any JS library) · *One API* (`from.to(...channels)`, tiny, tree-shakeable) · *Verified* (per-space cited anchors + colorjs.io differential + official ACES vendor transforms). Everything else is support.
...
|---|---|---|
| **162 color spaces** | runtime-verified count; categorized + cited in README | others ship ~15–40; none reach the long tail |
| **Camera-log + broadcast + appearance breadth** | S-Log3 (S-Gamut3 + S-Gamut3.Cine), LogC4, V-Log, RED Log3G10, Canon Log, Cineon, F-Log, BMD Film; YUV/YCbCr family; CAM16/CIECAM02/CAM-UCS/LCD/SCD, Hellwig 2022, ZCAM, HCT | not in culori/colorjs.io/chroma/texel at all |
...
| **Three-layer verification** | every space pinned to an independent cited conformance anchor (`test/refs.js`, 135 points); 29 spaces both directions vs colorjs.io at 1/255 (`test/reference.js`); **camera logs differential-tested against the Academy's official ACES vendor transforms** with published deltas (`test/aces-vendor.js`, docs/lut-verification.md); corrects published-paper errors | others self-test only; none test against official vendor transforms |
| **MCP server** | `npx color-space-mcp`: `convert` / `space` / `spaces` / `cube` tools — agents ground color math in verified conversions | no other color lib ships one |
...
1. **Reach any space — even the exotic ones.** *Stop hand-porting formulas from PDFs; the space you need is already here, cited.* (Proof: 162 spaces, the camera-log/appearance/Munsell long tail.)
...
4. **Stays small and unencumbered.** *Take one space, ship under 2 kB; no license to read.* (Proof: tree-shakeable ESM, zero deps; 0.4–1.5 kB per space, 55 kB gzip for all 162; public domain.)
...
1. **Every color space, one API, verified.** (162 · conventional ranges · anchored + differential-tested)
...
- **"162 spaces"** — runtime count; the categorized, source-cited list in the README is itself the proof of completeness.
- **"Verified"** — three layers, quote precisely: (1) every space pinned to an independent cited conformance anchor (135 points, test/refs.js); (2) 29 spaces differential vs colorjs.io (CSS spec editors), both directions, 1/255; (3) camera logs differential vs the Academy's official ACES vendor transforms, deltas published (docs/lut-verification.md). Plus documented paper corrections.
- **"CSS-native"** — side-by-side `lab(50,0,0)` vs others' `lab(0.5,0,0)`.
- **"Tiny / zero-dep"** — **quote the tree-shaken per-space import (0.4–1.5 kB)**, not the full bundle (all 162 = 55 kB min+gz — *larger* than culori/colorjs.io because it has 4× the spaces; never call the full library "tiny"). Comparing our per-space import to culori's/colorjs.io's *full* bundle is apples-to-oranges — compare like for like. v3.0.0 is on npm — the per-space numbers are measurable and current. No dependencies. (See [market-data.md](market-data.md).)
...
- `162 color spaces in JavaScript — with values that match CSS.`
- `The color-space conversion library with the space you actually need.`
- `Convert any color space to any other — 162 of them, formulas verified against the CSS spec.`
- Wedge (film): `The only JavaScript library that speaks S-Log3, ACES, and Rec.2100.`
- Wedge (colorist): `Free .cube conversion LUTs for every camera log — verified against the Academy's official ACES transforms, deltas published.`
