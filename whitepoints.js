/**
 * Whitepoint reference values — CIE standard illuminants (A, B, C, D50-D75, F1-F12,
 * E) for both the 1931 2° and 1964 10° observers, XYZ scaled to Y = 100. Split out
 * of xyz.js so that spaces which never touch the table (most of the library) don't
 * carry it in tree-shaken bundles — import it only when you need an illuminant.
 *
 *     import whitepoint from 'color-space/whitepoints.js'
 *     whitepoint[2].D50   // → [96.422, 100, 82.521]
 *
 * @see {@link https://en.wikipedia.org/wiki/Standard_illuminant}
 */
const whitepoint = {
  // 1931 2° Observer
  2: {
    // Incandescent
    A: [109.85, 100, 35.585],
    B: [99.0927, 100, 85.313],
    C: [98.074, 100, 118.232],
    D50: [96.422, 100, 82.521],
    D55: [95.682, 100, 92.149],
    // Daylight
    D65: [95.04559270516717, 100, 108.90577507598783], // full precision (matches sRGB matrix + colorjs)
    D75: [94.972, 100, 122.638],
    // Fluorescent
    F1: [92.834, 100, 103.665],
    F2: [99.187, 100, 67.395],
    F3: [103.913, 100, 65.710],
    F4: [109.147, 100, 38.813],
    F5: [90.872, 100, 98.723],
    F6: [97.309, 100, 60.191],
    F7: [95.044, 100, 108.755],
    F8: [96.413, 100, 82.333],
    F9: [100.365, 100, 67.868],
    F10: [96.174, 100, 81.712],
    F11: [100.966, 100, 64.370],
    F12: [108.046, 100, 39.228],
    // Equal Energy
    E: [100, 100, 100]
  },

  // 1964 10° Observer
  10: {
    // Incandescent
    A: [111.144, 100, 35.200],
    B: [99.178, 100, 84.349],
    C: [97.285, 100, 116.145],
    D50: [96.720, 100, 81.427],
    D55: [95.799, 100, 90.926],
    // Daylight
    D65: [94.811, 100, 107.304],
    D75: [94.416, 100, 120.641],
    // Fluorescent
    F1: [94.791, 100, 103.191],
    F2: [103.280, 100, 69.026],
    F3: [108.968, 100, 66.500],
    F4: [114.961, 100, 40.963],
    F5: [93.369, 100, 98.636],
    F6: [102.148, 100, 62.074],
    F7: [95.792, 100, 107.687],
    F8: [97.115, 100, 81.135],
    F9: [102.116, 100, 67.826],
    F10: [99.001, 100, 83.134],
    F11: [103.866, 100, 65.627],
    F12: [111.428, 100, 40.353],
    // Equal Energy
    E: [100, 100, 100]
  }
};

export default whitepoint;
export { whitepoint };
