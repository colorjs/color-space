...
export const catHTML = () =>
	...
	${c.spaces.map(s => { const cls = classify(s)
		return `<article class="ent" data-s="${s}">
		 ...
		 <div class="chs">${cls.ch.map((c2, i) => `<div class="ch" data-i="${i}"><div class="tk"></div><span class="sy">${c2.sym}<i class="cn">${cname(c2)}</i></span><span class="nvw"><input class="nv" type="number" step="${step(c2)}" min="${c2.min}" max="${c2.max}" title="${cname(c2)}" aria-label="${s} ${cname(c2)}"><i class="u">${unit(c2)}</i></span></div>`).join('')}</div>
		</article>` }).join('')}
