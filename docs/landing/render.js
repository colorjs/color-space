...
const step = c => (c.max - c.min) <= 1 ? 0.001 : (c.max - c.min) <= 40 ? 0.01 : (c.max - c.min) <= 110 ? 0.1 : 1
...
export const catHTML = () =>
	`<nav class="toc" id="toc">${sections.map((c, i) => `<i class="tsp"></i><button class="ti" data-i="${i}">${c.name}<span class="c tnum">${c.spaces.length}</span></button>`).join('')}</nav><div class="secs" id="secs">` + sections.map(c => `<section class="sec" data-sec><h2 class="shw"><button class="sh" aria-expanded="false">${c.name}<span class="c tnum">${c.spaces.length}</span><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 9l6 6 6-6"/></svg></button></h2><button class="mo" aria-expanded="false" title="all ${c.name} spaces" aria-label="show all ${c.name} spaces"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 9l6 6 6-6"/></svg></button><div class="grid">
	${c.spaces.map(s => { const cls = classify(s)
		return `<article class="ent" data-s="${s}">
		 ...
		 <div class="chs">${cls.ch.map((c2, i) => `<div class="ch" data-i="${i}"><div class="tk"></div><span class="sy">${c2.sym}</span><span class="nvw"><input class="nv" type="number" step="${step(c2)}" min="${c2.min}" max="${c2.max}" title="${cname(c2)}" aria-label="${s} ${cname(c2)}"><i class="u">${unit(c2)}</i></span></div>`).join('')}</div>
		</article>` }).join('')}
