...
### WASM premise — RE-MEASURED (jz 0.8.1, Node/V8, 1M px, zero-copy buffer-in-WASM, optimize:'speed', checksum-forced)
  ...
  * [ ] **Stage the remaining transcendental families** — HDR (jzazbz/jzczhz/ictcp), camera logs (logc4/slog3/vlog/log3g10/clog2), din99d/ipt/hct. Edges already generated + validated by the workflow (≤1e-5; in the task output) — just need the module-scope constants assembled into batch.js + graph wiring.
  * [ ] **Optimizable cases fed to jz's bench** (per "optimize the tool, not the input"): `bench/colorlch` (fused rgb→oklch is ~parity with V8 but the split is 1.6× — jz should schedule the fused mixed-transcendental body as well as the split). Add more as found.
