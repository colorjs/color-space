...
// inverse — Fairchild notes it must be reversed by successive approximation — so
// hunt.xyz runs a damped Newton on the forward in (J, C·cos h, C·sin h) space.
...
hunt.xyz = (J, C, h) => {
	// damped Newton on the forward in (J, C·cos h, C·sin h) space (no hue wrap)
	const hr = h * Math.PI / 180, T = [J, C * Math.cos(hr), C * Math.sin(hr)];
	const g = p => {
		const [j, c, hh] = xyz.hunt(...p), a = hh * Math.PI / 180;
		return [j, c * Math.cos(a), c * Math.sin(a)];
	};
	let p = [95.05 * J / 100, 100 * J / 100, 108.88 * J / 100], best = p, bestE = Infinity;
	for (let i = 0; i < 60; i++) {
		const f = g(p), e = [f[0] - T[0], f[1] - T[1], f[2] - T[2]];
		const err = Math.hypot(...e);
		if (err < bestE) { bestE = err; best = p; }
		if (err < 1e-10) break;
		const hstep = 1e-4, Jm = new Array(9);
		for (let c = 0; c < 3; c++) {
			const pp = p.slice();
			pp[c] += hstep;
			const fp = g(pp);
			for (let r = 0; r < 3; r++) Jm[r * 3 + c] = (fp[r] - f[r]) / hstep;
		}
		const dp = mat3(inv3(Jm), -e[0], -e[1], -e[2]);
		if (!dp.every(Number.isFinite)) break;
		let lam = 1, pNew = null;
		for (let s2 = 0; s2 < 10; s2++) {
			const cand = p.map((v, i2) => Math.max(-20, Math.min(250, v + lam * dp[i2])));
			const fc = g(cand);
			if (Math.hypot(fc[0] - T[0], fc[1] - T[1], fc[2] - T[2]) < err) { pNew = cand; break; }
			lam /= 2;
		}
		if (!pNew) break;
		p = pNew;
